...
import com.foxinmy.weixin4j.model.media.*;
...
import com.foxinmy.weixin4j.qy.model.*;
...
public class QyAPI {
    ...
    @PostConstruct
    private void init() {
        TokenCreator tokenCreator = new WeixinTokenCreator(APP_ID, APP_SECRET);
    ...
    private void initChat() {
        TokenCreator tokenCreator = new WeixinTokenCreator(APP_ID, CHAT_SECRET);
        TokenManager tokenManager = new TokenManager(tokenCreator, redisCacheStorager);
     ...
     * 获取客服列表
     *
     * @param kfType 客服类型 为空时返回全部类型的客服
     ...
     * 下载媒体文件
     *
     * @param agentid 企业应用Id(<font color="red">大于0时视为获取永久媒体文件</font>)
     * @param mediaId 媒体ID
     * @return 媒体下载结果
     * @throws WeixinException
     ...
     *
     * @param agentid   企业应用ID
     * @param mediaType 素材的类型，图片（image）、视频（video）、语音 （voice）、图文（news）、文件（file）
     * @param pageable  分页数据
     * @return 媒体素材的记录对象
     * @throws WeixinException
     * @see com.foxinmy.weixin4j.qy.api.MediaApi
     ...
     *
     * @param partyId 部门ID。获取指定部门ID下的子部门 传入0表示获取全部子部门
     * @return 部门列表
     * @throws WeixinException
     * @see com.foxinmy.weixin4j.qy.model.Party
     * @see <a href=
     ...
     * @param userStatus 成员状态 status可叠加 未填写则默认为未关注(4)
     * @param findDetail 是否获取详细信息
     * @return 成员列表
     * @throws WeixinException
     * @see com.foxinmy.weixin4j.qy.model.User
     * @see <a href=
     ...
     * 获取权限范围内的所有成员列表
     *
     * @param userStatus 成员状态 未填写则默认为全部状态下的成员
     * @return 成员列表
     * @throws WeixinException
     * @see com.foxinmy.weixin4j.qy.api.UserApi
     ...
     *
     * @param userId 成员ID
     * @param tips   推送到微信上的提示语（只有认证号可以使用）。当使用微信推送时，该字段默认为“请关注XXX企业号”，邮件邀请时，该字段无效。
     * @return 调用结果
     * @throws WeixinException
     * @see com.foxinmy.weixin4j.qy.api.UserApi
     * @see <a href=
     ...
     *
     * @param tag 标签对象；</br> 标签名称，长度为1~64个字节，标签名不可与其他标签重名；</br> 标签id，整型，
     ...
     *
     * @param tagId    标签ID
     * @param userIds  企业成员ID列表，注意：userlist、partylist不能同时为空
     * @param partyIds 企业部门ID列表，注意：userlist、partylist不能同时为空
     ...
     * 批量邀请成员关注
     *
     * @param parameter 成员ID,标签ID,部门ID
     * @param callback  接收任务执行结果的回调地址等信息
     * @param tips      推送到微信上的提示语（只有认证号可以使用）。当使用微信推送时，该字段默认为“请关注XXX企业号”，邮件邀请时，该字段无效。
     ...
     * </p>
     *
     * @param mediaId  带userid信息的cvs文件上传后的media_id
     * @param callback 接收任务执行结果的回调地址等信息
     * @return 异步任务id，最大长度为64字符
     * @throws WeixinException
     * @see com.foxinmy.weixin4j.qy.model.Callback
     ...
     * </p>
     *
     * @param mediaId  带partyid信息的cvs文件上传后的media_id
     * @param callback 接收任务执行结果的回调地址等信息
     * @return 异步任务id，最大长度为64字符
     * @throws WeixinException
     * @see com.foxinmy.weixin4j.qy.model.Callback
     ...
     * @param agentid 需要发送红包的应用ID，若只是使用微信支付和企业转账，则无需该参数 传入0或负数则忽略
     ...
     * 更新会话
     *
     * @param chatInfo    会话信息 至少保持会话ID不能为空
     * @param operator    操作人userid
     * @param addUsers    会话新增成员列表
     * @param deleteUsers 会话退出成员列表
     * @return 处理结果
     * @throws WeixinException
     * @see com.foxinmy.weixin4j.qy.api.ChatApi
     * @see com.foxinmy.weixin4j.qy.model.ChatInfo
     ...
     * 清除会话未读状态
     *
     * @param targetId 会话值，为userid|chatid，分别表示：成员id|会话id
     * @param owner    会话所有者的userid
     * @param chatType 会话类型：single|group，分别表示：群聊|单聊
     * @return 处理结果
     * @throws WeixinException
     * @see com.foxinmy.weixin4j.qy.api.ChatApi
     * @see <a href=
    ...
    public ApiResult sendGroupTextChatMessage(String chatId, String senderId, String messageText)
            throws WeixinException {
